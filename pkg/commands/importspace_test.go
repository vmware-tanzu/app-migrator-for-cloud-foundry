...
func TestImportSpace_Run(t *testing.T) {
	type fields struct {
		ImportOrg ImportOrg
		Space     string
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{ImportOrg: ImportOrg{Org: "my_org"}, Space: "my_space"},
			args: args{
				ctx: &context.Context{
		...
		{
			name:   "returns error when org and space do not exist",
			fields: fields{ImportOrg: ImportOrg{Org: "does_not_exist"}, Space: "does_not_exist"},
			args: args{
				ctx: &context.Context{
					ExportDir: filepath.Join(pwd, "testdata/apps"),
					Metadata:  metadata.NewMetadata(),
