...
func Test_exportSpace(t *testing.T) {
	fakeCommandRunner1 := new(fakes.FakeSpaceCommandRunner)
	fakeCommandRunner2 := new(fakes.FakeSpaceCommandRunner)
	fakeCommandRunner3 := new(fakes.FakeSpaceCommandRunner)
	type args struct {
		ctx         *context.Context
		runner      *fakes.FakeSpaceCommandRunner
		commandArgs []string
	}
	tests := []struct {
		name       string
	...
	}{
		{
			...
			args: args{
				...
				commandArgs: []string{"space", "some-space", "-o", "some-org", "--domains-to-replace", "foo.com=bar.com"},
			},
			beforeFunc: func() {
				fakeCommandRunner1.RunCalls(func(c *context.Context, org, space string) error {
		...
		},
		{
			...
			args: args{
				...
				commandArgs: []string{"space", "some-space", "-o", "some-org", "--domains-to-replace", "foo1.com=bar1.com,foo2.com=bar2.com"},
			},
			beforeFunc: func() {
				fakeCommandRunner2.RunCalls(func(c *context.Context, org, space string) error {
					require.Equal(t, "some-org", org)
					require.Equal(t, "some-space", space)
		...
		},
		{
			...
			args: args{
				...
				commandArgs: []string{"space", "some-space", "-o", "some-org", "--domains-to-replace", "foo1.com=bar1.com,foo2.com=bar1.com"},
			},
			beforeFunc: func() {
				fakeCommandRunner3.RunCalls(func(c *context.Context, org, space string) error {
					require.Equal(t, "some-org", org)
					require.Equal(t, "some-space", space)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			exportSpaceCmd := CreateExportSpaceCommand(tt.args.ctx, tt.args.runner)
